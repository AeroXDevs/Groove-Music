...
module.exports = {
    ...
    async slashExecute(interaction, client) {
        ...
        if (!interaction.member.permissions.has('ManageChannels') && !isOwner) {
            const display = new TextDisplayBuilder().setContent(client.t(interaction.guildId, "mod.needPermShort", { e: client.emoji.cross, permission: "Manage Channels" }));
    ...
    async execute(message, args, client) {
        ...
        if (!message.member.permissions.has('ManageChannels') && !isOwner) {
            const display = new TextDisplayBuilder().setContent(client.t(message.guild.id, "mod.needPermShort", { e: client.emoji.cross, permission: "Manage Channels" }));
