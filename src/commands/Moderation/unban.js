...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (args.length === 0) {
            const header = new TextDisplayBuilder().setContent(client.t(message.guild.id, "mod.header.unban", { e: emoji.info, user: message.author.username, ts: Math.floor(Date.now() / 1000) }));
