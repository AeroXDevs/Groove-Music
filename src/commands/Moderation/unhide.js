...
module.exports = {
    ...
    async slashExecute(interaction, client) {
        ...
        try {
            ...
            if (isVisible) {
                const display = new TextDisplayBuilder().setContent(client.t(interaction.guildId, "mod.chan.alreadyVisible", { e: emoji.warn, channel }));
                return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
            }
            ...
            const display = new TextDisplayBuilder().setContent(client.t(interaction.guildId, "mod.chan.visible", { e: emoji.check, channel }));
            return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        } catch (error) {
            const display = new TextDisplayBuilder().setContent(client.t(interaction.guildId, "mod.failed.unhide", { e: emoji.warn, message: error.message }));
            return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        }
    },
    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            if (isVisible) {
                const display = new TextDisplayBuilder().setContent(client.t(message.guild.id, "mod.chan.alreadyVisible", { e: emoji.warn, channel }));
                return message.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
            }
            ...
            const display = new TextDisplayBuilder().setContent(client.t(message.guild.id, "mod.chan.visible", { e: emoji.check, channel }));
            return message.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        } catch (error) {
            const display = new TextDisplayBuilder().setContent(client.t(message.guild.id, "mod.failed.unhide", { e: emoji.warn, message: error.message }));
            return message.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        }
    }
};
