...
module.exports = {
    ...
    async runSimilar(context, client, isSlash) {
        ...
        if (!player || !player.queue.current) {
            const errorDisplay = new TextDisplayBuilder().setContent(client.t(guildId, "music.nothingPlaying", { e: client.emoji.cross }));
        ...
        if (tracks.length === 0) {
            const noResultsDisplay = new TextDisplayBuilder().setContent(client.t(guildId, "music.similar.none", { e: client.emoji.cross, engine: searchEngine }));
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === 'sim_prev') {
                ...
                const successDisplay = new TextDisplayBuilder()
                    .setContent(client.t(guildId, "music.similar.added", { e: client.emoji.check, title: track.title, uri: track.uri }));
                ...
                const successDisplay = new TextDisplayBuilder()
                    .setContent(client.t(guildId, "music.similar.addedAll", { e: client.emoji.check, count: tracks.length }));
                const successContainer = new ContainerBuilder().addTextDisplayComponents(successDisplay);
